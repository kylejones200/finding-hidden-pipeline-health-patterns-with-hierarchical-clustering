...
def visualization_2_compressor_operational_regimes_d(N, Z_health, logger) -> None:
    ...
    for i in range(12, 15):
        idx = slice(i * 24, (i + 1) * 24)
        flow[idx] += np.random.choice([-30, 0, 30], 24, p=[0.15, 0.7, 0.15])
    ...
    df_row.loc[forest_indices, "ndvi_mean"] = np.random.uniform(0.65, 0.8, 40)
    df_row.loc[forest_indices, "ndvi_std"] = np.random.uniform(0.1, 0.2, 40)
    df_row.loc[forest_indices, "bare_soil_fraction"] = np.random.uniform(0.0, 0.1, 40)
    ...
    df_row.loc[bare_indices, "bare_soil_fraction"] = np.random.uniform(0.6, 0.85, 25)
...
def main() -> None:
    ...
    visualization_2_compressor_operational_regimes_d(N, Z_health, logger)
